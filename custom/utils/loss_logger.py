...
def read_loss_config() -> dict:
    ...
    try:
        ...
        raise ValueError(f"YOLO_IOU_INNER_RATIO='{ratio_raw}' is not a valid float.")
    if not (0 < inner_ratio <= 1.0):
        raise ValueError(f"YOLO_IOU_INNER_RATIO={inner_ratio} must be in (0, 1].")
...
def log_loss_config(trainer) -> None:
    ...
    LOGGER.info(
        ...
        + (
            f"\n  iou_mean    : {iou_mean_str} (EMA buffer)"
            if iou_mean_str != "n/a"
            else ""
        )
        + f"\n{sep}\n"
